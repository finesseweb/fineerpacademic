        ...
        switch ($type) {
            ...
            case 'edit':
                ...
                if(empty($result['result_of_exam'])){
                    ...
                    $Actualcgpa= round($result['total_grade']/$result['total_credit'],2);
                    
                    $percent= !empty($Actualcgpa)?$Actualcgpa*10:'100';
                    //echo '<pre>'; print_r($Actualcgpa);exit;
                    $ref_grade_item = new Application_Model_ReferenceGradeMasterItems();
                    $letter_grade = $ref_grade_item->getRecordByNumGrade($percent,$result['degree_id'],$result['session']);
                  
                    $result['result_of_exam']    .= 'Grade ('.$letter_grade.')';
                    $result['result_of_exam']    .= " with CGPA: ".  $Actualcgpa;
                    }
                }
...
//                
    ...
    public function getStudentPassOutCardAction(){
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($stream) {
                $erpStudent_model = new Application_Model_StudentPortal();
                    $result = $passOut_model->getStudentsForPassOut($stream,$session,$academic_id);
                $this->view->corecourseresult = $result;
            }
        }
    }
    ...
    public function passoutprintAction(){
        ...
        switch($applicantDetails['degree']){
            ...
            case 2:
                ...
                if (in_array($applicantDetails['academic_id'], $mcaThreeYrs)) {  $cmn_terms = 't6';} 
                else{ $cmn_terms = 't4';}    
            break;
            case 3:
                $cmn_terms = 't4';
        ...
        if(isset($applicantDetails['total_grade'])){
...
public function ajaxGetExportBySessionAction(){
    ...
    public function semesterFeeCollectionAction(){
        $this->view->action_name = 'studentinfo';
        $this->view->sub_title_name = 'charachterstudent';
