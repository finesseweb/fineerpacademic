...
//Core Course Learning 
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($_POST['csrftoken'])) {
                if ($_POST['csrftoken'] === $token) {
                    ...
                    unset($data['academic_year']);
                    unset($data['tot_credit']);
                    $checkExistedData=$Corecourselearning_model->checkExistedData($data['mig_academic_id'],$data['mig_term_id']);
                    //echo '<pre>'; print_r($checkExistedData);exit;
                    if(empty($checkExistedData)){
                     foreach ($data['course_id'] as $key => $course) {
                         $migData= array(
                                    'academic_year_id' =>  $data['mig_academic_id'],   
                                    'term_id' =>  $data['mig_term_id'],   
                                    
                                    'course_id' => $data['course_id'][$key],
                                    'course_code' => $data['course_code'][$key],
                                    'cc_id' => $data['cc_id'][$key],
                                    'credit_id' => $data['credit_id'][$key],
                                    're_credit'=> $data['re_credit'][$key],
                                    'ge_id' => $_POST['ge_id'][$key],
                                    'count_id' => $_POST['count_id'][$key],
                                    'degree_id' => $_POST['degree_id'][$key],
                                    'cmn_terms' => $_POST['cmn_terms'][$key]
    ...
    public function ajaxGetCoreCourseMasterAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $result = $cclModel->getprogramByYearTerm($academic_year_id, $term_id);
            $paginator_data = array(
                'page' => $page,
                'result' => $result
            );
...
//    }
    ...
    public function ajaxGetBulkSavedResultAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $credit_no = $this->_getParam("credit_no");
            $credit_id = $this->_getParam("credit_id");
           
           // echo '<pre>';print_r($credit_id);exit;
            foreach ($credit_no as $key => $value) {
                $data = array();
                
                $data['earned_credit'] = $value;
                $student_db->update($data, array('student_id=?' => $credit_id[$key]));
            }
            die;
        }
    }
...
//Based On Short Code need to display Program Name	
    ...
    public function ajaxGetAcademicOnStreamAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $academic_model = new Application_Model_Academic();
            $getAcademicId = $academic_model->getAcademicOnDept($getDepartment['did']);
            //$result = $academic_model->getRecordBySessionId($session_id);
            ...
            case "add":
                if ($this->getRequest()->isPost()) {
                    if ($ReferenceGradeMaster_form->isValid($this->getRequest()->getPost())) {
                        ...
                        if (!empty($data['csrftoken'])) {
                            if ($data['csrftoken'] === $token) {
                                $bool = $ReferenceGradeMaster_model->getExitstingRecord($data['academic_year_id'], $data['degree_id'], $data['session']);
