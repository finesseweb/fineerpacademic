...
//        }
    ...
    //    $term_result = $term_master->getTermRecordsByYear($academic_id, $year_id);
      ...
      //------------------[bulk term_id]---------------//  
        ...
        else if(!empty($term_id))
        {
            ...
            $result = $this->getReports($stu_id,$academic_id,$year_id,$value['term_id'],$arc);
    ...
    public function attendancereportAction(){
        ...
        $attendance_result =  $attendance_info->getAttendanceOnMonthRange1($department_id,$course_id,$ge_id,$cc_id,$term_id, $monthFrom, $monthTo);
        ...
        $ids = $this->mergData($attendance_result,array('attendance_master_id'),count($attendance_result));
          ...
          $distinct_date = $attendance_info->distinctDate($department_id,$course_id,$ge_id,$cc_id,$term_id, $monthFrom, $monthTo);
    ...
    public function attendancereportViewAction(){
		...
		if($this->_request->isPost () && $this->getRequest ()->isXmlHttpRequest ()){
            ...
            if(empty($attendance_result)){
                 echo '<pre>'; print_r('<b style="color:red">Record Not Found Between Selected Date!</b>');exit;
            }else{
...
//             $term_id = $term_details->getTermRecordsbycmn($academic_id,$term_id)['term_id'];
    ...
    public function getStudentMarks($academic_id, $tabulation_id, $stu_id, $term_id,$arc_status=false) {
...
public function ajaxGetNonpgDetailsAction() {
      ...
      public function getStudentAdmitcardAction() {
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($academic_id) {
                $Studentreport_model = new Application_Model_StudentPortal();
                $result = $Studentreport_model->getStudentadmitRecord($academic_id, $stu_id,$term_id);
    ...
    public function ajaxGetFeesInstallmentRecordAction(){
     ...
     public function ajaxGetEndSemesterRecordAction(){
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $attendance = $this->_getParam("attend");
            $totexamfee = $examfeeStr->getEndFee($academic_id);
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
          ...
          //   print_r($strId);die();
           ...
           // echo '<pre>';print_r($semFee);exit;
            ...
            $result = $backitem->getStudentForNonColRecords($academic_id,$sem);
            $paginator_data = array(
                'result' => $result,
    ...
    public function passFailReportAction(){
        ...
        switch ($type) {
            ...
            case "edit":
                ...
                $result = $academic_model->getCoreCourseByCourseId($edit_id,$session,$cmnTerms);
                //echo"<pre>";print_r($result);exit;
                //$departmentName=$department_model->getIndividualDepartmentType($edit_id,$session);
                    //echo '<pre>';print_r($result);exit;
