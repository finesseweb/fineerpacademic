...
class BatchAttendanceController extends Zend_Controller_Action {
            ...
            case "add":
                if ($this->getRequest()->getPost()) {
                    ...
                    if($data['csrftoken']===$token ){
                        ...
                        $checkExistedData= $batchAttendanceModel->checkMonthlyAttendance($data['session'],$data['cmn_terms'],$data['degree_id'],$data['effective_date'],$data['cc_id'],$data['ge_id'],$data['department'],$data['course_id'],$employee_id);
            ...
            case "add":
                if ($this->getRequest()->getPost()) {
                    ...
                    if($data['csrftoken'] === $token ){
                        ...
                        $checkExistedData=$batchSemesterAttendanceModel->isexist($data['session'],$data['cmn_terms'],$data['cc_id'],$data['department'],$data['course_id'],$data['ge_id']);
    ...
    public function ajaxCalculateMonthlyAttendanceAction(){
        ...
        if ($this->_request->isPost() && $this->getRequest()->isXmlHttpRequest()) {
            ...
            $checkExistedData= $batchAttendanceModel->checkMonthlyAttendance($session,$term_id,$degree_id,$effective_date,$cc_id,$ge_id,$hons_id,$course_id,$employee_id);
            if(!empty($checkExistedData)){
                ...
                foreach ($academic_id_Arr as $key => $value) {
                    if ($value) {
                        if($degree_id!=1){
                            ...
                            $newData[$value] = $StudentPortal_model->getstudentsdetailattend($value,$term_id,$tabId);  
                            
                            if($course_id){
                            $raw_data[$value] = $course_learning->getStudentsForElectiveByCourse($academic_id_Arr[$key], $course_id, $termidarr[$key],'electives',$tabId);
