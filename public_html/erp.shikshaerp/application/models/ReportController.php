...
//        }
			...
			$StudentPortal_model = new Application_Model_ElectiveSelectionItems();
                        ...
                        if($course_id !=0){
			$result = $StudentPortal_model->getelectivestudentDetails1($academic_year_id,$term_id,explode('_',$course_id)[0]);}
			else
			{
			    
			    $student_model = new Application_Model_StudentPortal();
			    $result=$student_model->getstudentsbyacademics($academic_year_id,$term_id);
			}
    ...
    public function attendancereportAction(){
        ...
        $attendance_result =  $attendance_info->getAttendanceOnMonthRange1($department_id,$course_id,$term_id, $monthFrom, $monthTo);
        ...
        $ids = $this->mergData($attendance_result,array('attendance_master_id'),count($attendance_result));
          ...
          $distinct_date = $attendance_info->distinctDate($department_id,$course_id, $term_id, $monthFrom, $monthTo);
      ...
      //------------------[bulk term_id]---------------//  
       ...
       //---------------------[END]-------------------------// 
        ...
        //===================[GRADE SHEET NUMBER]===============================//
                ...
                $this->view->gradesheet_number = $gradesheet_number;
        //------------------------------[Print Option According to Mode]-------------------//
         $htmlcontent = $this->view->render('report/back-paper-report.phtml');
         if ($check == 'admin' || $mode == 'view') {
                echo $htmlcontent;
                exit;
            }//======for PDF
