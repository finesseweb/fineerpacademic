...
page(_($help_context = "Remove Items"), false, false, "", $js);
...
{
	$id = $_GET['AddedID'];
   	display_notification(_("The Room Item has been Issued."));
...
if (isset($_POST['addissue']) && can_process())
{
	...
	$failed_data = add_return_issues($_POST['stock_id'],
		$_POST['sl_no'], $_POST['name'],$_POST['floor'] ,
		$_POST['room'], $_POST['department'], $_POST['seat'],$_POST['date_'], $_POST['status'],$_POST['memo_'],$_POST['return_status'],"R",$_POST['bid'],$_POST['Location']);

        
  ///  header("Location:manufacturing/manage/manufactured_items.php")  ;
        ///display_error($failed_data);
//	if ($failed_data != null) 
//	{
...
//	else 
//	{
//		//meta_forward($_SERVER['PHP_SELF'], "AddedID=".$_POST['room']);
//	}
    $Ajax->redirect('manage/manufactured_items.php?closed=closed');
...
start_form();
//display_search_details('R');

div_start('controls', 'items_table');
//display_room_requirements($_POST['room'],'');
div_end();
$Ajax->activate('items_table');
echo "<br>";
...
//stock_component_items_list_cells(null, 'stock_id', null, $stock_id, false, true);
//display_issue_itemsm(_("Items to Issue"), $stock_id);
// display_error($stock_id);

remove_manufacture_items($stock_id,$bid,$comp,$loc,$location);
echo "</td></tr>";

end_table();

submit_center('addissue', _("Remove"), true, '', 'default');
